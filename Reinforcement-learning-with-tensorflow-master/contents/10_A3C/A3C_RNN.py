...
class ACNet(object):
    ...
    def _build_net(self, scope):
        ...
        with tf.compat.v1.variable_scope('critic'):   # only critic controls the rnn update
            cell_size = 64
            s = tf.expand_dims(self.s, axis=1,
                               name='timely_input')  # [time_step, feature] => [time_step, batch, feature]
            rnn_cell = tf.compat.v1.contrib.rnn.BasicRNNCell(cell_size)
        ...
        with tf.compat.v1.variable_scope('actor'):  # state representation is based on critic
